...
public class CG2020P1 {
	...
	try {
		...
		if(cases<1||cases>10000000)
			return;
		long caseExecute =0;
		long [] ingredients = new long[cases];
		long [] ingredientsUnits = new long[cases];
		for(int i=0;i<cases;i++)
	    {
				caseExecute = reader.nextLong();
				if(caseExecute<1||caseExecute>Long.MAX_VALUE)
				{
					return;
				}
				else
				{
					ingredients[i] = caseExecute;
				}
	    }
		for (int j = 0; j < cases; j++) 
		{
			...
			{
				ingredientsUnits[j] = caseExecute;
			}
		}
	
		System.out.println(getMaxUnits(ingredients, ingredientsUnits));
	}
	finally {
		reader.close();
	}
	...
	private static long getMaxUnits(long[] ingredients, long[] ingredientsUnits) 
	{
		long Min = -1;
		for (int i = 0; i < ingredients.length; i++) {
			if(ingredients[i]==0 || ingredientsUnits[i]==0)
				return 0;
			long remainder = ingredientsUnits[i]/ingredients[i];
			if(i==0)
			{
				Min = remainder;
			}
			if(i!=0 && remainder<Min)
			{
				Min= remainder;
			}
		}
