...
The tickets were provided to each house with an integer written on it.Some got tickets with positive integers and some got tickets with negative integers. 
...
Also, he don't wish to become bad in front of people. So, he came up with an idea to design a program which will provide 
the list of integers forming maximum sum and thus all the members of the list will be given the gifts. 
...
the list with first greater element would be considered. For better understanding, look at the explanation of 
Test case 4 in Sample Test Case. The tickets with integer 0 are not considered for winning the gifts.
...
public class CG2 {
	...
	public static void main(String[] args) {
		...
		for(int i=0;i<cases;i++)
	    {
				...
				if(housesCount<1||housesCount>10000)
					...
					seq = new int[ticketArray.length]; 
					bestSeq=new int[ticketArray.length];
					maxSum(ticketArray);
					System.out.println();
	    }
}
	//11 12 -2 -1
	//152 0 5 7 -1 0 8 4 1 3 -2	==  [152, 152, 157, 159, 159, 159, 167, 167, 168, 170, 170]
	//-1 7 8 -5 4 ==  	 [-1, 7, 7, 7, 11]
	//3 2 1 -1 ==			 [3, 3, 4, 4] 
	//4 5 4 3   ==		 [4, 5, 8, 8]
	//5 10 4 -1  	==		 [5, 10, 10, 10]
	...
	public static void printOutput(int bestSeq[])
	{
		for (int j = 0; j < bestSeq.length; j++) 
		{
			if(bestSeq[(bestSeq.length-1)-j]!=0)
				System.out.print(bestSeq[(bestSeq.length-1)-j]);	
		}
	}
